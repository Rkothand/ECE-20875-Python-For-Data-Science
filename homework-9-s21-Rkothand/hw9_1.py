...
def getFormattedText(filename) :
    ...
    for l in lines:
        paddedLines.append('__'+l.lower().strip()+'__')
...
def getDict(filename):
    ...
    txt=getFormattedText(filename)

    ngrams = list(map(getNgrams,txt))
    for g in ngrams:
        for i in g:
            nGramDict[i]= nGramDict.get(i,0)+1  
    return nGramDict
...
def topNCommon(filename,N):
    ...
    commonN = []
    dictN = getDict(filename)
    sortedN = (sorted(dictN.items(), key=lambda item: item[1], reverse=True))
    commonN=sortedN[0:N]    
...
def getAllDicts(fileNamesList):
    ...
    for i in fileNamesList:
        langDicts.append(getDict(i))
    return langDicts
 # txt = getFormattedText(fileNamesList)
...
def dictUnion(listOfDicts):
    ...
    for i in listOfDicts:    
        for k,v in i.items():
            if(k not in unionNGrams):
                unionNGrams.append(k)
    unionNGrams.sort()       
...
def getAllNGrams(langFiles):
    allNGrams = []
    a=getAllDicts(langFiles)
    allNGrams=dictUnion(a)
...
def compareLang(testFile,langFiles,N):
    langMatch = ''
    testN = set(t[0] for t in topNCommon(testFile,N))

    common = []
    for i in langFiles:
        langN = set(n[0] for n in topNCommon(i,N))
        common.append((len(testN.intersection(langN)),i))
    common.sort(reverse=True)
       


    langMatch = common[0][1]
